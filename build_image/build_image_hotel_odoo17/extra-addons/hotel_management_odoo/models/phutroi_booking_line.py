...
class PhuTroiBookingLine(models.Model):
    """Model that handles the service booking form"""
    _name = "phutroi.booking.line"
    _description = "Hotel phụ trội Line"
    ...
    booking_id = fields.Many2one("room.booking", string="Booking",
                                 ...
                                 ondelete="cascade")
    phutroi_id = fields.Many2one('hotel.phutroi', string="Phụ Trội",
                                 help="Indicates the Service")
    description = fields.Char(string='Description', related='phutroi_id.name',
    ...
    tax_ids = fields.Many2many('account.tax',
                               'hotel_phutroi_order_line_taxes_rel',
                               'phutroi_id', 'tax_id',
                               related='phutroi_id.taxes_ids', string='Taxes',
                               help="Default taxes used when selling the "
                                    "services.",
                               domain=[('type_tax_use', '=', 'sale')])

    @api.onchange('phutroi_id')
    def _onchange_phutroi_id(self):
        if self.phutroi_id:
            self.price_unit = self.phutroi_id.unit_price
