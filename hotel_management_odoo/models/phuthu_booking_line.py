...
class PhuThuBookingLine(models.Model):
    """Model that handles the service booking form"""
    _name = "phuthu.booking.line"
    _description = "Hotel phụ thu Line"
    ...
    booking_id = fields.Many2one("room.booking", string="Booking",
                                 ...
                                 ondelete="cascade")
    phuthu_id = fields.Many2one('hotel.phuthu', string="Phụ Thu",
                                 help="Indicates the Service")
    description = fields.Char(string='Description', related='phuthu_id.name',
    ...
    tax_ids = fields.Many2many('account.tax',
                               'hotel_phuthu_order_line_taxes_rel',
                               'phuthu_id', 'tax_id',
                               related='phuthu_id.taxes_ids', string='Taxes',
                               help="Default taxes used when selling the "
                                    "services.",
                               domain=[('type_tax_use', '=', 'sale')])

    @api.onchange('phuthu_id')
    def _onchange_phuthu_id(self):
        if self.phuthu_id:
            self.price_unit = self.phuthu_id.unit_price
